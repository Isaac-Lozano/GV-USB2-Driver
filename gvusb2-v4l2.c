...
void gvusb2_vid_clear_queue(struct gvusb2_vid *dev)
{
    ...
    if (dev->current_buf) {
        vb2_buffer_done(&dev->current_buf->vb.vb2_buf, VB2_BUF_STATE_ERROR);
...
static int gvusb2_vb2_start_streaming(struct vb2_queue *vb2q, unsigned int count)
{
    int ret;
    struct gvusb2_vid *dev = vb2_get_drv_priv(vb2q);
    s32 reg_07;
...
static int gvusb2_s_ctrl(struct v4l2_ctrl *ctrl) {
    struct gvusb2_vid *dev = container_of(ctrl->handler, struct gvusb2_vid, ctrl_handler);

    switch(ctrl->id) {
    case V4L2_CID_BRIGHTNESS:
        i2c_smbus_write_byte_data(&dev->i2c_client, 0x10, (ctrl->val + 0x80) & 0xff);
        break;
    case V4L2_CID_CONTRAST:
        i2c_smbus_write_byte_data(&dev->i2c_client, 0x11, ctrl->val);
        break;
    case V4L2_CID_SATURATION:
        i2c_smbus_write_byte_data(&dev->i2c_client, 0x13, ctrl->val);
        i2c_smbus_write_byte_data(&dev->i2c_client, 0x14, ctrl->val);
        break;
    case V4L2_CID_HUE:
        i2c_smbus_write_byte_data(&dev->i2c_client, 0x15, (ctrl->val + 0x80) & 0xff);
        break;
    case V4L2_CID_SHARPNESS:
        i2c_smbus_write_byte_data(&dev->i2c_client, 0x12, (ctrl->val & 0x0f) | 0x50);
        break;
    }
...
static int gvusb2_vidioc_querycap(struct file *file, void *priv, struct v4l2_capability *cap)
{
...
static int gvusb2_vidioc_enum_input(struct file *file, void *priv, struct v4l2_input *i)
{
    struct gvusb2_vid *dev = video_drvdata(file);
...
static int gvusb2_vidioc_querystd(struct file *file, void *priv, v4l2_std_id *std)
{
    struct gvusb2_vid *dev = video_drvdata(file);
...
static int gvusb2_vidioc_g_std(struct file *file, void *priv, v4l2_std_id *std)
{
    ...
    if (vb2_is_busy(vb2q)) {
        gvusb2_dbg(&dev->intf->dev, "i am busy\n");
    }
...
static int gvusb2_vidioc_enum_fmt_vid_cap(struct file *file, void *priv, struct v4l2_fmtdesc *f)
{
    struct gvusb2_vid *dev = video_drvdata(file);
...
static int gvusb2_vidioc_g_fmt_vid_cap(struct file *file, void *priv, struct v4l2_format *f)
{
    struct gvusb2_vid *dev = video_drvdata(file);
    int width, height;
...
static int gvusb2_vidioc_s_fmt_vid_cap(struct file *file, void *priv, struct v4l2_format *f)
{
    struct gvusb2_vid *dev = video_drvdata(file);
    struct vb2_queue *vb2q = &dev->vb2q;
    int width, height;
...
static int gvusb2_vidioc_try_fmt_vid_cap(struct file *file, void *priv, struct v4l2_format *f)
{
    struct gvusb2_vid *dev = video_drvdata(file);
    int width, height;
